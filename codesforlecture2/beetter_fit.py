...
trd = np.append(trd,[np.power(trd1,3)/10],axis=0)# scale them so that they may not create overflow error
trd = np.append(trd,[np.power(trd2,3)/10],axis=0)
trd = np.append(trd,[(trd1)*(np.power(trd2,2))/10],axis=0)
trd = np.append(trd,[(np.power(trd1,2))*(trd2)/10],axis=0)
...
F = theta[0]*X + theta[1]*Y +theta[2]*X*X +theta[3]*Y*Y +theta[4]*X*Y+theta[5]*X*X*X +theta[6]*Y*Y*Y + theta[7]*X*Y*Y +theta[8]*X*X*Y +theta[9]
